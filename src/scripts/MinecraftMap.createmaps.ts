...
module MinecraftMap {
	...
	export function createMapImageInDiv(zoomLevelNumber, divElementName, aWidth, aHeight, config, locations, finishedCallback) {
		...
		var newImage = <HTMLImageElement>$(document.createElement('img')).css('display', 'block')[0];
	...
	// returns the name of the map
	 function CreateAreaMapInDiv(divElementName, aWidth, aHeight, config, locations){
		...
		for (index = 0; index < locations.length; ++index) {
			...
			var newArea = <HTMLAreaElement>document.createElement('area');
	...
	function drawMapDetails(canvas, config, locations, labellingStyle)
	{
		...
		function locationLabel_bounds(locationInstance, finalizedCaption, pixelOffsetFromLocation_y: number) {
		...
		function multilineCenteredText_bounds(x: number, y: number, text: string, padding?: number) {
	...
	function setCanvasScalingToPixelated(ctx, makePixelated?: boolean) {
	...
	export function PreRender(config) {
		...
		if (gOceanMapImage != null) {
			...
			var mapBackgroundImage = <HTMLImageElement>document.getElementById('map-background');
